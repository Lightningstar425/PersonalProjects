...
class ChannelCapture:
    ...
    def handle_response(self, response):
        ...
        try:
            ...
            return  # not JSON, or body unavailable -- skip
        ...
        if any(ep in url for ep in MESSAGE_ENDPOINTS):
            ...
            for msg in msg_list:
                if isinstance(msg, dict) and msg.get("ts"):
                    self.messages[msg["ts"]] = msg  # dedup + overwrite on edits
        ...
        for msg in msg_list:
            ...
            if thread_ts and thread_ts != ts:
                self.threads[thread_ts][ts] = msg
            else:
                self.messages[ts] = msg
...
history_requests = {"count": 0}

def on_response(response):
    if "conversations.history" in response.url:
        history_requests["count"] += 1



def scroll_to_top(page, capture, max_passes=200):
    pane = page.locator(MESSAGE_PANE_SELECTOR)

    last_count = capture.history_requests
    ...
    for i in range(max_passes):
        ...
        print(
            f"pass={i} "
            f"history_requests={capture.history_requests}"
        )

        if capture.history_requests == last_count:
            break
...
def export_channel(page, url, index):
    ...
    try:
        ...
        scroll_to_top(page, capture)
        time.sleep(1.5)  # let any final in-flight requests finish
    finally:
    ...
    for ts, msg in sorted(
        capture.messages.items(),
        key=lambda kv: float(kv[0])
    ):
        uid = msg.get("user") or msg.get("bot_id")
        user = capture.users.get(
            msg.get("user"),
            msg.get("username") or uid or "unknown"
        )

        text = clean_text(msg.get("text"), capture.users)

        lines.append(
            f"**[{ts_to_iso(ts)}] {user}:** {text}"
        )

        replies = capture.threads.get(ts, {})

        for r_ts, reply in sorted(
            replies.items(),
            key=lambda kv: float(kv[0])
        ):
            r_uid = reply.get("user") or reply.get("bot_id")

            r_user = capture.users.get(
                reply.get("user"),
                reply.get("username") or r_uid or "unknown"
            )
            ...
            lines.append(
                f"    **[{ts_to_iso(r_ts)}] {r_user}:** {r_text}"
            )
